...
local function build_internal(params)
	...
	do -- memory
		...
		local apom_dep, apom_parts
		local apom_part, apom_add_dep
		do
			apom_dep = {}
			function apom_add_dep(name, dep)
				if not apom_dep[name] then
					apom_dep[name] = {}
				end
				apom_dep[name][dep] = true
			end

			apom_parts = {}
			function apom_part(p, name, ...)
				for i = 1, select("#", ...) do
					apom_add_dep(name, select(i, ...))
				end
				-- local pp = part(p)
				-- TODO
				apom_parts[name] = p
		...
		do -- horizontal axis
			...
			apom_part({ type = pt.PSTN, x = x_left -  5, y = y_bank, extend = math.huge, debug_dcolour = 0xFFFF0000 }, "vert_read_out", "horiz_copy_2")
			...
			apom_add_dep("vert_read_ldtc1", "vert_read_address")
			apom_add_dep("vert_read_ldtc2", "vert_adjust_1")
			apom_add_dep("vert_read_ldtc3", "vert_read_ldtc2")
			apom_add_dep("vert_write_dray", "vert_write_address")
		...
		local y_usage      = y_body + height + 30
		local y_usage_next = y_usage + 20
		local x_cray       = x_body - 29
		local x_ballast    = x_body + 4
		local x_ballast_2  = x_body + 90
		...
		local apom_puts = {}
		local address_source = part({ type = pt.FILT, x = x_body - 3, y = y_usage - 10, tmp = 1, ctype = 0x1000DEAD })
		do -- usage site
			...
			local sources = {}
			local function apom_put_get(name, move_existing)
				...
				local p = sources[key]
				apom_add_put(dray(p_src.x, y_usage, p_src.x, p_src.y, 1, pt.PSCN), name)

				local cr
				if p_src.type == pt.CRAY then
					cr = dray(p_src.x, y_usage_next, p_src.x, p_src.y, 1, pt.PSCN)
				else
					cr = cray(p_src.x, y_usage_next, p_src.x, p_src.y, pt.SPRK, 1, pt.PSCN)
				end
			...
			apom_put_get("get_head_parts", true)
			...
			do
				local cr = cray(x_head_parts, y_usage, x_head_parts, y_head_parts + 4, pt.HEAC, 4, pt.PSCN)
			...
			do
				...
				local cr1 = cray(type_query_filt.x - 2, y_usage_next    , type_query_filt.x - 2, type_query_filt.y - 1, pt.HEAC, 1, pt.PSCN)
				local cr2 = cray(type_query_filt.x    , y_usage_next    , type_query_filt.x    , type_query_filt.y - 1, pt.HEAC, 2, pt.PSCN)
				...
				part ({ type = pt.LSNS, tmp = 3, x = temp_target.x, y = y_usage_next - 1 })
				cray(temp_target.x, y_usage_next - 1, temp_target.x, temp_target.y, pt.HEAC, 1, false)
			...
			local to_visit = {}
			local seen = {}
			-- TODO: get rid of next, pairs
			local function try_visit(name)
				if not seen[name] and (not apom_dep[name] or not next(apom_dep[name])) then
					to_visit[name] = true
					seen[name] = true
					count = count + 1
					apom_order[name] = count
					-- print("ts", name)
				end
			end
			for name in pairs(apom_parts) do
				try_visit(name)
			end
			while next(to_visit) do
				local name = next(to_visit)
				to_visit[name] = nil
				for next_name in pairs(apom_parts) do
					if apom_dep[next_name] then
						apom_dep[next_name][name] = nil
						try_visit(next_name)
					end
				end
			end
			...
			for ix_put = #apom_puts, 1, -1 do
				...
				local put_order = apom_order[put.name]
			...
			-- lsns_spark({ type = pt.PSCN, x = x_ballast - 1, y = y_usage_next, life = 3 }, 1, -1, 2, -1)
			...
			do
				local x_write = x_body + 90
				ldtc(x_write + 1, y_usage, x_storage_slot(7), y_usage)
				dray(x_write + 1, y_usage, x_write + 4, y_usage, 1, pt.INST)
				ldtc(x_write + 1, y_usage, x_storage_slot(5), y_usage)
				dray(x_write + 1, y_usage, x_write + 7, y_usage, 1, pt.INST)
				part({ type = pt.FILT, x = x_write +  2, y = y_usage })
				part({ type = pt.STOR, x = x_write +  3, y = y_usage })
				part({ type = pt.STOR, x = x_write +  5, y = y_usage })
				part({ type = pt.FILT, x = x_write +  6, y = y_usage, ctype = 1, tmp = 1 })
				dray(x_write + 9, y_usage, x_dray, y_usage, 1, pt.METL, nil, true)
				part({ type = pt.STOR, x = x_write +  9, y = y_usage })
				part({ type = pt.DMND, x = x_write + 11, y = y_usage })
				aray(x_write + 1, y_usage, -1, 0, pt.INST, nil, 988)
				aray(x_write + 1, y_usage, -1, 0, pt.INST, nil, 988) -- sets tmp to 1
				dray(x_write + 1, y_usage, x_write + 10, y_usage, 1, pt.INST)

				dray(x_ldtc_2 + 2, y_usage, x_write +  4, y_usage, 1, pt.PSCN)
				dray(x_ldtc_2 + 2, y_usage, x_write +  7, y_usage, 1, pt.PSCN)
				dray(x_ldtc_2 + 2, y_usage, x_write + 10, y_usage, 1, pt.PSCN)
			end
			do
		...
		do -- cleanup
			local y_cleanup = y_body + height + 80

			for ix_put = 1, #apom_puts do
				local x_cray = apom_get_x_ballast(ix_put)
				cray(x_cray, y_cleanup + ix_put % 2, x_cray, y_usage_next, pt.HEAC, 1, pt.PSCN)
				cray(x_cray, y_cleanup + ix_put % 2, x_cray, y_usage     , pt.HEAC, 1, pt.PSCN)
			end
		end
		...
		do -- decode address
			...
			part({ type = pt.CONV, x = x_decode + 1, y = y_decode + 1, tmp = pt.SPRK, ctype = pt.CRMC, z = -1000 })
			...
			cray(x_decode, y_decode, last_piston.x, last_piston.y, pt.STOR, width_order + height_order    , false)
			cray(x_decode, y_decode, last_piston.x, last_piston.y, pt.STOR, width_order + height_order - 1, false)
			change_conductor(pt.METL)
			local function handle_bit(address_bit, piston_bit, piston_index, last)
			...
			for i = 0, width_order - 1 do
				...
				handle_bit(width_order - i + 1, width_order - i - 1, piston_index, false)
				local piston = pistons[piston_index]
			...
			for i = 0, height_order - 1 do
				...
				handle_bit(width_order + 1 + height_order - i, height_order - i, piston_index, i == height_order - 1)
				local piston = pistons[piston_index]
				local stagger = (piston.x + 1) % 2
